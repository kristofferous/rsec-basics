 ...
 */
export abstract class BaseRepository<T extends { required: Record<string, any> }> {
    /**
     ...
     * @param pool
     */
    protected constructor(public createInstance: (row: any) => T, private pool: mysql.Pool) {
    }
    ...
    async fetchAllBy(column: string, value: any): Promise<T[]> {
        const rows = await this.query(`SELECT *
                                  FROM ${this.tableName()}
                                  WHERE ${column} = ?`, [value]);
        return rows.map((row: any) => this.createInstance(row));
    }
     ...
     */
    async fetchOne(...q: UUID[]):Promise<T | undefined> {
    ...
    async insert(data: T['required']): Promise<T> {
        ...
        await this.query(`INSERT INTO ${this.tableName()} (${keys}) VALUES (${placeholders})`, [...values]).catch(e => {
            if (e.errno === 1062) throw new DuplicateError(this.tableName(), data); else throw e
        });
        ...
        for (const key in data) {
            if (data.hasOwnProperty(key)) {
                await this.query(`UPDATE ${this.tableName()} SET ${key} = ? WHERE id = ?`, [data[key], id]).catch(e => {
                    if (e.errno === 1062) throw new DuplicateError(this.tableName(), {[key]: data[key]});
                });
            }
        }
